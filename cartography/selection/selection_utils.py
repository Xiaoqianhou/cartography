  ...
  for epoch_num in tqdm.tqdm(range(num_epochs)):
    ...
    with open(epoch_file, "r") as infile:
      for line in infile:
        ...
        if record[id_field] == 3:
            continue
        guid = record[id_field] if not strip_last else int(str(record[id_field])[:-1])
        #if this data is in the original dev data , then draw the condition of it
        if guid <= 104743:
            continue
